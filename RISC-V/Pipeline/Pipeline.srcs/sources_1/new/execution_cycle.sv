...
module execution_cycle(
    ...
    // Combinational
    mux3 forward_a_mux(.d0(rd1_e), .d1(result_w), .d2(alu_result_m_in), .s(forward_a_e), .y(src_a_e));
    mux3 forward_b_mux(.d0(rd2_e), .d1(result_w), .d2(alu_result_m_in), .s(forward_b_e), .y(write_data_e_wire));

    mux2 alu_src_mux(.d0(write_data_e_wire), .d1(imm_ext_e), .s(alu_src_e), .y(src_b_e));

    alu arithmetic_logic_unit(.a(src_a_e), .b(src_b_e), .alu_ctrl(alu_ctrl_e), .result(alu_result_e_wire), .zero(zero_e));
    adder_branch branch_adder(.pc(pc_e), .imm(imm_ext_e), .target(pc_target_e));

    assign pc_src_e = (branch_e & zero_e) | jump_e;

    // Embedded E -> M Pipeline Register
